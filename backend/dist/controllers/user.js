...
export const addUser = TryCatch(async (req, res, next) => {
    ...
    const hashUserId = await bcrypt.hash(userId, 10);
    ...
    if (permissions && permissions.length > 0) {
        const permissionQueries = permissions.map(({ publicationId, editionId, permission }) => {
            return conn.query("INSERT INTO user_permission (User_Id, Publication_Id, Edition_Id, permission) VALUES (?, ?, ?, ?)", [hashUserId, publicationId, editionId, permission]);
        });
...
export const getUser = TryCatch(async (req, res, next) => {
    ...
    const conn = await getConnection();
    const [user] = await conn.query("SELECT User_Name FROM user WHERE User_Id = ?", [userId]);
    if (!user) {
