...
export const getUser = TryCatch(async (req, res, next) => {
    ...
    const [user] = await conn.query(
        "SELECT User_Name, Email FROM user WHERE User_Id = ?",
        [userId]
    );
...
export const addOrUpdateUserPermission = TryCatch(async (req: Request<{ userId?: string }, {}, { permissions: PermissionType[] }>, res, next) => {
    const { userId } = req.params;
...
export const getAllUsers = TryCatch(async (req, res, next) => {
    ...
    const permissionsResults = await conn.query(
        "SELECT User_Id, Publication_Id, Edition_Id, permission FROM user_permission"
    );
