...
const EMagazine = () => {
  ...
  useEffect(() => {
    const fetchPublications = async () => {
      try {
        const response = await axios.get('/magazines/get-publication');
  ...
  const handleSubmit = async () => {
      ...
      try {
        ...
        for (const row of rows) {
          if (parseInt(row.from) > parseInt(row.to)) {
            throw new Error("Page From must be less than or equal to Page To.");
          }
        }
    
        for (let i = 1; i < rows.length; i++) {
          const prevTo = parseInt(rows[i - 1].to);
          const currFrom = parseInt(rows[i].from);
    
          if (currFrom <= prevTo) {
            throw new Error(`Page From in row ${i + 1} must be greater than Page To in row ${i}`);
          }
        }
